...
const NewAddition = (props: Pick<FieldDiffMetada, "newValue">) => {
  return (
    <>
      <Badge className="h-auto" variant="lb_green">
        {props.newValue}
      </Badge>
    </>
  );
...
const Deletion = (props: Pick<FieldDiffMetada, "oldValue">) => {
  return (
    <>
      <Badge className="h-auto" variant="red" withDot={true}>
        {props.oldValue}
      </Badge>
    </>
  );
  ...
  return (
    <ListItem rounded={false} className="block flex-col border-0 md:border-0">
      <div className="flex flex-wrap items-center justify-between">
        <div className="flex max-w-[65%] flex-col items-start p-1">
        ...
        <Button
          className="ml-auto shrink-0 self-end rounded-md text-xs"
          color="secondary"
          size="sm"
          disabled={isLoading}
